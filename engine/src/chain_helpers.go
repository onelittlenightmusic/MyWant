...
// GetInputChannel returns input channel by index Returns (nil, false) if not available, (channel, true) if available 
...
// GetOutputChannel returns output channel by index Returns (nil, false) if not available, (channel, true) if available 
...
// GetFirstInputChannel returns the first input channel from paths Returns (nil, false) if no input channels available, (channel, true) if available 
...
// GetFirstOutputChannel returns the first output channel from paths Returns (nil, false) if no output channels available, (channel, true) if available 
...
// SetPaths sets both input and output paths for this want This is the proper way to synchronize paths instead of direct field access 
...
// Returns: (channelIndex, data, ok) - channelIndex: Index of the channel that provided data (-1 if no data available) - data: The data received (nil if ok is false) - ok: True if data was successfully received, false otherwise
// 
...
func (n *Want) ReceiveFromAnyInputChannel(timeoutMilliseconds int) (int, interface{}, bool) {
	...
	// Build select cases dynamically for all input channels from paths IMPORTANT: Must track the original channel index mapping!
