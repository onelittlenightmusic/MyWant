...
func (k *KnowledgeWant) GetKnowledgeStatus() string {
	return GetCurrent(k, "knowledge_status", "stale")
}
...
func (k *KnowledgeWant) IsAchieved() bool {
	return k.GetKnowledgeStatus() == "fresh"
}
...
func (k *KnowledgeWant) shouldRefresh() bool {
	locals := k.GetLocals()
	lastSyncTime := GetCurrent(k, "last_sync_time", "")
	if lastSyncTime == "" {
		return true
	}
	lastTime, err := time.Parse(time.RFC3339, lastSyncTime)
	if err != nil {
		return true
	}
