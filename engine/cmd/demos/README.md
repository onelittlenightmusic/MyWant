...
```sh
go run ./engine/cmd/demos/debug_simple
go run ./engine/cmd/demos/demo_flight
```

Top-level placeholders

The files remaining directly under `engine/cmd/demos` are simple placeholder files in `package demos_moved` that point to the new locations. This avoids having multiple `package main` files in this directory and keeps the repository buildable.
