...
type queue struct { time_last_out float; in chan float; out chan float; stat }
type path struct { vpath chan float; end chan bool; }

func newpath () (path) {
	return path { make (chan float), make (chan bool) }
}
func newasyncpath(n int) (path) {
	return path { make (chan float, n), make (chan bool) }
}
...
func filter(in path, p float) (path) {
	out := newpath();
	_p := p;
	switch {
	case _p > 1: _p = 1
...
func generate(i int, d float) (path) {
	out := newpath();
	go func() {
		t := 0.0;
		L : for j:=0; j<i; j++ {
			t += d*float(rand.ExpFloat64());
			select {
			case <- out.end: break L;
			case out.vpath <- t:
...
func display_log (q *queue) () {
	ave:=q.interval_sum/float(q.out_num);
	fmt.Printf("{\n");
	fmt.Printf("\tAve:\t%f\n", ave);
	fmt.Printf("\tVar:\t%f\n", q.interval_sqsum/float(q.out_num)-ave*ave);
	fmt.Printf("\tQueued Num Ave:\t%f\n", float(q.queued_num)/float(q.out_num));
	fmt.Printf("\tNum:\t%d\n", q.out_num);
	fmt.Printf("}\n");
}
func main() {
